#include "ronny.h"
// #include "g/keymap_combo.h"

// clang-format off
#define LAYOUT_sofle_base( \
    K01, K02, K03, K04, K05, K06, K07, K08, K09, K0A, \
    K11, K12, K13, K14, K15, K16, K17, K18, K19, K1A, \
    K21, K22, K23, K24, K25, K26, K27, K28, K29, K2A  \
    ) \
    LAYOUT_wrapper( \
        KC_TAB,   ________________NUMBER_LEFT________________,               ________________NUMBER_RIGHT_______________,       KC_DEL,     \
        CTL_ESC,  K01,      K02,      K03,      K04,      K05,                       K06,     K07,     K08,     K09,     K0A,   KC_BSPC,    \
        KC_LSFT , K11,      K12,      K13,      K14,      K15,                       K16,     K17,     K18,     K19,     K1A,   KC_QUOT,    \
        KC_LCTL,  K21,      K22,      K23,      K24,      K25, KC_MUTE,     KC_MPLY, K26,     K27,     K28,     K29,     K2A,   KC_BSLS,    \
                    PLOVER, KC_LALT,  KC_LGUI,  SYM,      KC_SPC,                 KC_ENT,     NAV,     KC_MINS, COPY,    PASTE              \
    )

#define LAYOUT_sofle_base_wrapper(...) LAYOUT_sofle_base(__VA_ARGS__)

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {

    /* ,-----------------------------------------.                    ,-----------------------------------------.
    * | TAB  |  1   |  2   |  3   |  4   |  5   |                    |  6   |  7   |  8   |  9   |  0   |DEL⌦  |
    * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
    * |ESC/⎈ |  Q   |  W   |  E   |  R   |  T   |                    |  Y   |  U   |  I   |  O   |  P   |BSPC �|
    * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
    * |LSFT ⇧|  A   |  S   |  D   |  F   |  G   |,------.    ,------.|  H   |  J   |  K   |  L   | : ;  | ' "  |
    * |------+------+------+------+------+------|| MUTE |    |  ⏯   ||------+------+------+------+------+------|
    * |LCTL ⎈|  Z   |  X   |  C   |  V   |  B   |`------'    `------'|  N   |  M   | , <  | . >  | ? /  | | \  |
    * `-------------+------+------+------+-.------------.     ,-----------.-+------+------+------+-------------'
    *               |LCTL ⎈|LALT⎇ |LGUI◆ |/ SYM  / SPC  /    \ ENT  \ NAV  \| - _  | ⧉    |  📋  |
    *               |      |      |      /      /      /      \      \      \      |      |      |
    *               `---------------------------------'        `---------------------------------'
    *                                                                                   generated by [keymapviz] */
    [_QWERTY] = LAYOUT_sofle_base_wrapper(
        _________________QWERTY_L1_________________, _________________QWERTY_R1_________________,
        _________________QWERTY_L2_________________, _________________QWERTY_R2_________________,
        _________________QWERTY_L3_________________, _________________QWERTY_R3_________________
    ),

    /* ,----------------------------------------.                    ,-----------------------------------------.
    * | TAB  |  1   |  2   |  3   |  4   |  5   |                    |  6   |  7   |  8   |  9   |  0   |DEL⌦  |
    * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
    * |ESC/⎈ |  Q   |  W   |  F   |  P   |  B   |                    |  J   |  L   |  U   |  Y   | : ;  |BSPC �|
    * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
    * |LSFT ⇧|  A   |  R   |  S   |  T   |  G   |,------.    ,------.|  M   |  N   |  E   |  I   |  O   | ' "  |
    * |------+------+------+------+------+------|| MUTE |    |  ⏯   ||------+------+------+------+------+------|
    * |LCTL ⎈|  Z   |  X   |  C   |  D   |  V   |`------'    `------'|  K   |  H   | , <  | . >  | ? /  | | \  |
    * `-------------+------+------+------+-.------------.    ,------------.-+------+------+------+-------------'
    *               |LCTL ⎈|LALT⎇|LGUI◆|/ SYM   / SPC  /     \ ENT  \ NAV  \| - _  | ⧉    |  📋  |
    *               |      |      |    /       /      /       \      \      \      |      |      |
    *               `---------------------------------'        `---------------------------------'
    *                                                                                   generated by [keymapviz] */
    [_COLEMAKDH] = LAYOUT_sofle_base_wrapper(
        ________________COLEMAK_DH_L1______________, ________________COLEMAK_DH_R1______________,
        _____________COLEMAK_DH_HMR_L2_____________, _____________COLEMAK_DH_HMR_R2_____________,
        ________________COLEMAK_DH_L3______________, ________________COLEMAK_DH_R3______________
    ),

    [_PLOVER] = LAYOUT_wrapper(
        _______, _________________STENOG_L1_________________,                    _________________STENOG_R1_________________, KC_BSPC,
        _______, _________________STENOG_L2_________________,                    _________________STENOG_R2_________________,  STN_DR,
        _______, _________________STENOG_L3_________________,                    _________________STENOG_R3_________________,  STN_ZR,
        _______, OS_LGUI, OS_LALT, OS_LSFT, OS_LCTL, _______, _______,  _______, _______, OS_RCTL, OS_RSFT, OS_LALT, OS_RGUI, _______,
                  PLOVER, _______, STN_A, STN_O,     TAB_SYM,                    ENT_NAV,  STN_E,  STN_U,   _______, _______
    ),

    /* ,-----------------------------------------.                    ,-----------------------------------------.
    * |KC_F12|KC_F1 |KC_F2 |KC_F3 |KC_F4 |KC_F5 |                    |KC_F6 |KC_F7 |KC_F8 |KC_F9 |KC_F10|KC_F11|
    * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
    * |  `   |KC_1  |KC_2  |KC_3  |KC_4  |KC_5  |                    |KC_6  |KC_7  |KC_8  |KC_9  |KC_0  |BSPC �|
    * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
    * |  ~   |O_PRQO|  {   |  [   |  (   |  =   |,------.    ,------.|  +   |  )   |  ]   |  }   |C_PRQO|\ |   |
    * |------+------+------+------+------+------||      |    |      ||------+------+------+------+------+------|
    * |      |  !   |  @   |  #   |  $   |  %   |`------'    `------'|  ^   |  &   |  *   | ["   | "]   |  |   |
    * `-------------+------+------+------+-.------------.    ,------------.-+------+------+------+-------------'
    *               |      |      |      |/      /      /    \      \      \|      |      |      |
    *               |      |      |      /      /      /      \      \      \      |      |      |
    *               `---------------------------------'        `---------------------------------'
    *                                                                                   generated by [go-keymapviz] */
    [_SYM] = LAYOUT_wrapper(
        KC_F12,   __________________FUNC_L1__________________,                    __________________FUNC_R1__________________,  KC_F11,
        KC_GRV,   ________________NUMBER_LEFT________________,                    ________________NUMBER_RIGHT_______________,  KC_BSPC,
        KC_TILD,  _________________SYMBOL_L2_________________,                    _________________SYMBOL_R2_________________,  KC_BSLS,
        _______,  _________________SYMBOL_L3_________________, _______,  _______, _________________SYMBOL_R3_________________,  KC_PIPE,
                  ___________________TRANS___________________,                    ___________________TRANS___________________
    ),

    /* ,-----------------------------------------.                    ,-----------------------------------------.
    * |KC_F12|KC_F1 |KC_F2 |KC_F3 |KC_F4 |KC_F5 |                    |KC_F6 |KC_F7 |KC_F8 |KC_F9 |KC_F10|KC_F11|
    * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
    * |LCTL �|CS_T  |CS_TAB|C_T   |C_W   |C_TAB |                    |MOUSE |MOUSE |MOUSE |MOUSE |      |KC_BTN|
    * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
    * |      |KC_WSC|KC_BAC|      |      |KC_FWD|,------.    ,------.|KC_LEF|KC_DOW|KC_UP |KC_RGH|KC_PGU|KC_BTN|
    * |------+------+------+------+------+------||      |    |      ||------+------+------+------+------+------|
    * |      |KC_WH_|WHEEL |WHEEL |KC_WH_|      |`------'    `------'|VIM_B |KC_END|KC_HOM|VIM_W |KC_PGD|RSFT_T|
    * `-------------+------+------+------+-.------------.    ,------------.-+------+------+------+-------------'
    *               |      |      |      |/      /      /    \      \      \|      |      |      |
    *               |      |      |      /      /      /      \      \      \      |      |      |
    *               `---------------------------------'        `---------------------------------'
    *                                                                                   generated by [go-keymapviz] */
    [_NAV] = LAYOUT_wrapper(
        KC_F12,   __________________FUNC_L1__________________,                    __________________FUNC_R1__________________,  KC_F11,
        KC_LCTL,  __________________TAB_NAV__________________,                    _________________VIM_ARROW_________________,  KC_BTN1,
        _______,  __________________WWW_NAV__________________,                    ___________________ARROW___________________,  KC_BTN2,
        _______,  ________________MOUSE_WHEEL________________, _______,  _______, _________________MOUSE_NAV_________________,  _______,
                  ___________________TRANS___________________,                    ___________________TRANS___________________
    ),

    /*,-----------------------------------------.                    ,-----------------------------------------.
    * |      |TD(BOO|TD(QWE|TD(COL|PANIC |      |                    |EE_CLR|PANIC |      |      |      |      |
    * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
    * |      |      |      |      |      |      |                    |      |      |      |      |      |      |
    * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
    * |      |NumLoc|ScrLoc|Ins   |PrintS|CW_TOG|,------.    ,------.|KC_LEF|KC_DOW|KC_UP |KC_RGH|KC_PGU|      |
    * |------+------+------+------+------+------||      |    |      ||------+------+------+------+------+------|
    * |      |  ↶   |  ↷   |  ✂  |  ⧉   | 📋    |`------'    `------'|  ⏮   |KC_VOL|KC_VOL| ⏭     | ⏯   |      |
    * `-------------+------+------+------+-.------------.    ,------------.-+------+------+------+-------------'
    *               |      |      |      |/      /      /    \      \      \|      |      |      |
    *               |      |      |      /      /      /      \      \      \      |      |      |
    *               `---------------------------------'        `---------------------------------'
    *                                                                                   generated by [go-keymapviz] */
    [_ADJUST] = LAYOUT_wrapper(
        _______,  _________________ADJUST_L1_________________,                    _________________ADJUST_R1_________________,  _______,
        _______,  ___________________EMPTY___________________,                    _________________ADJUST_R2_________________,  _______,
        _______,  _________________ADJUST_L2_________________,                    ___________________ARROW___________________,  _______,
        _______,  _________________ADJUST_L3_________________, _______,  _______, _________________ADJUST_R3_________________,  _______,
                  ___________________TRANS___________________,                    ___________________TRANS___________________
    )
};
// clang-formatter on
